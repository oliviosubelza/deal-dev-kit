...
## 11. Skill `general/tdd` (test-driven development, opt-in)

Skill nueva adaptada de `github.com/obra/superpowers` (`test-driven-development`), vía
el fork `github.com/olivio-git/superpowers`. **Cambio de fondo respecto al original:** el
skill de superpowers impone el ciclo ("Iron Law", "NO PRODUCTION CODE WITHOUT A FAILING
TEST", "delete means delete"). Acá es **opt-in por tarea**: antes de escribir código de
implementación, el agente dice en una línea qué aporta test-first y **pregunta** si se
escribe el test primero. "No" cierra el tema para esa tarea; no se insiste.

### Archivos

| Archivo | Qué es |
|---|---|
| `skills/general/tdd/SKILL.md` | `name: general-tdd`, `applies_to: [backend, web, mobile]`, grupo "Team conventions". El ofrecimiento + el ciclo RED-GREEN-REFACTOR adaptado a prosa concisa (sin grafos `dot` ni tags `<Good>/<Bad>`). |
| `skills/general/tdd/writing-good-tests.md` | Referencia compañera, adaptada del `writing-good-tests.md` de superpowers: los dos principios (nombrar el break, ejercitar lo real), gate functions, mutation check, warning signs. Se quitó la cross-ref a `superpowers:writing-skills` y los "your human partner". El SKILL.md la linkea. |

`src:` apunta al **directorio** `skills/general/tdd`, así que el CLI copia los dos
archivos. Verificado con el binario real (`add general/tdd --dry-run`): planea
`crear .claude/skills/general-tdd/SKILL.md` + `.../writing-good-tests.md`, exit 0.

### Wiring en `kit.yaml`

- Artifact nuevo tras `general/security`: `{ id: general/tdd, type: skill, applies_to:
  [backend, web, mobile], src: skills/general/tdd, group: "Team conventions" }`. Sin
  `npm`.
- Agregado a los **tres** perfiles (`backend`, `web`, `mobile`), después de
  `general/security`.

### Por qué no hizo falta regenerar goldens

Los goldens de la TUI (`filter.golden`, `menu.golden`) se generan de un manifiesto
sintético de fixture, **no** del `kit.yaml` real. Solo `repo_manifest_test.go` y
`TestRealCatalogIsNavigable` cargan el manifiesto real, y este último no tiene golden —
solo asserts de conteo (`items < 50`, menú ≤ 6, grupos ≤ 12), que una skill más no
mueve. `go test ./internal/tui/ -count=1` pasa sin `-update`. (Los `M` en
`filter.golden`/`menu.golden` del working tree son de la línea "Engram para Claude Code"
de §10, no de este cambio.)

### Verificación

`gofmt -l .` limpio · `go vet ./...` limpio · `go test ./... -count=1` 12/12 paquetes ·
`go test ./internal/kit/ -count=1` (valida el `kit.yaml` real: perfiles resuelven, `src`
existe, `name` del frontmatter = id aplanado) OK · binario real `add --dry-run` OK.

### Tag

`kit-v*` (cambio en `skills/` + `kit.yaml`, no toca `tool/`).
